...
import {SortConfiguration} from 'smart-table-sort';
...
export interface SmartTable<T> extends Emitter {
    ...
    sort(input: SortConfiguration<T>): void;
    ...
    eval(tableState?: TableState<T>): Promise<DisplayedItem<T>[]>;
...
export interface TableConfiguration<T> {
    ...
    sortFactory: (conf: SortConfiguration<T>) => (array: T[]) => T[];
    filterFactory: (conf: FilterConfiguration) => (array: T[]) => T[];
    searchFactory: (conf: SearchConfiguration) => (array: T[]) => T[];
}

export default <T>({sortFactory, tableState, data, filterFactory, searchFactory}: TableConfiguration<T>): SmartTable<T> => {
